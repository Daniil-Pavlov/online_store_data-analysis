-- 2.2.1
--  Расчет на основе таблицы user_actions метрики CAC для двух рекламных кампаний.
...
WITH all_cl AS (SELECT time,user_id,order_id,action,
                ...
                10059, 10067, 10069, 10073, 10075, 10078, 10079, 10081, 10092, 10106, 10110, 10113, 10131) THEN 'Кампания № 2' END AS ads_campaign
                FROM user_actions) 
SELECT ads_campaign, ROUND((SUM(price)-250000)/250000*100,2) AS roi
FROM all_cl
LEFT JOIN (SELECT order_id,unnest(product_ids) AS product_id FROM orders) AS ord
USING (order_id)
LEFT JOIN products
USING (product_id)
WHERE ads_campaign IS NOT NULL AND order_id NOT IN (SELECT order_id FROM user_actions WHERE action = 'cancel_order')
...
FROM (  SELECT ads_campaign,user_id,AVG(roi) AS avg_check
        FROM (  SELECT ads_campaign,user_id,order_id, SUM(price) AS roi
                ...
                LEFT JOIN products
                USING (product_id)
                WHERE ads_campaign IS NOT NULL AND order_id NOT IN (SELECT order_id FROM user_actions WHERE action = 'cancel_order') AND time::DATE BETWEEN '2022-09-01' AND '2022-09-07'
                GROUP BY ads_campaign,user_id,order_id) AS x
...
FROM (  SELECT time,user_id,order_id,action,
        MIN(time) OVER (PARTITION BY user_id)::DATE AS start_date,
        DATE_TRUNC('month',MIN(time) OVER (PARTITION BY user_id))::DATE AS start_month,
        time::DATE - (MIN(time) OVER (PARTITION BY user_id)::DATE) AS day_number
...
FROM (  SELECT ads_campaign,
        ...
        time::DATE - MIN (time) OVER (PARTITION BY user_id ORDER BY time)::DATE AS day_number,
        user_id
        FROM all_cl
        WHERE ads_campaign IS NOT NULL) AS x
...
WITH all_cl AS (SELECT time,user_id,order_id,action,
                time::DATE - MIN (time) OVER (PARTITION BY user_id ORDER BY time)::DATE AS day_number,
                MIN (time) OVER (PARTITION BY user_id ORDER BY time) AS start_date,
                CASE WHEN user_id IN (8631, 8632, 8638, 8643, 8657, 8673, 8706, 8707, 8715, 8723, 8732, 8739, 8741, 
                8750, 8751, 8752, 8770, 8774, 8788, 8791, 8804, 8810, 8815, 8828, 8830, 8845, 
                8853, 8859, 8867, 8869, 8876, 8879, 8883, 8896, 8909, 8911, 8933, 8940, 8972, 
                8976, 8988, 8990, 9002, 9004, 9009, 9019, 9020, 9035, 9036, 9061, 9069, 9071, 
...
SELECT ads_campaign,CONCAT('Day ', day_number) AS day, 
ROUND(SUM (sm) OVER (PARTITION BY ads_campaign ORDER BY day_number range between unbounded preceding and current row )/c_u,2) AS cumulative_arppu, cac

FROM (  SELECT  ads_campaign,day_number,
                SUM (price) FILTER (WHERE order_id NOT IN (SELECT order_id FROM user_actions WHERE action = 'cancel_order')) AS sm
        FROM all_cl
                LEFT JOIN ( SELECT order_id,product_id,price
                            FROM (SELECT order_id, unnest(product_ids) AS  product_id FROM orders) AS ord
                            LEFT JOIN products
                            USING (product_id)) AS pp
                USING (order_id)           
